    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "" && textBox2.Text != "" && textBox3.Text != "" && textBox4.Text != "" &&
                textBox5.Text != "" && pamOne_tb.Text != "" && pamTwo_tb.Text != ""
                && class_tb.Text != "")
            {
                ...
                if (size >= 1 && m > 0 && pA > 0 && pA < m && pC > 0 && pC < m && sT > 0 && sT < m)
                {
                    LinearCongruention lc = new LinearCongruention(size, pA, pC, m, sT);
                    int[] tab = lc.kongruencja_liniowa(size, pA, pC, m, sT);
                    float[] tabf = new float[size];

                    for (int i = 0; i < size; i++)
                    {
                        tabf[i] = ((float) tab[i] / m);
                        //  tabf[i] = tab[i];
                    }
                        ...
                        if (pamTwo > pamOne)
                        {
                            ...
                            if (ChiTestQ_rb.Checked)
                            {
                                ...
                                if (isGoodEdgeClasses)
                                {
                                    PearsonTest pear = new PearsonTest(size, edgeClasses, edgeClasses.Length, tabFloat);
                                    int[] tabCount = pear.licznoscKlas(size, tabFloat, edgeClasses.Length, edgeClasses);
                                    float[] tabPearJed = pear.licznoscTeoretycznaJed(size, edgeClasses.Length, edgeClasses,
                                        pamOne, pamTwo);
                                    float stat = pear.statystykaTestowaPearson(edgeClasses.Length, tabPearJed, tabCount);
                                    float quantile = pear.Kwantyl(edgeClasses.Length, lvlofDeg);
                    ...
                    else if (ChiQ_rb.Checked)
                    {
                        ...
                        if (pamOne >= 1)
                        {
                            if (size % 2 == 0 || pamOne % 2 == 0)
                            {
                                ...
                                tab = lc.kongruencja_liniowa(newsize, pA, pC, m, sT);
                                float[] tabf2 = new float[newsize];

                                for (int i = 0; i < newsize; i++)
                                {
                                    tabf2[i] = ((float)tab[i] / m);
                                }
                        ...
                        if (pamOne > 0 && pamOne < 1)
                        {
                            ...
                            if (ChiTestQ_rb.Checked)
                            {
                                ...
                                if (isGoodEdgeClasses)
                                {
                                    ...
                                    int[] tabCount = pear.licznoscKlas(size, tabFloat, edgeClasses.Length, edgeClasses);
                                    float[] tabPearJed =
                                        pear.licznoscTeoretycznaZeroOne(size, edgeClasses.Length, edgeClasses, pamOne);
                                    float stat = pear.statystykaTestowaPearson(edgeClasses.Length, tabPearJed, tabCount);
                                    float quantile = pear.Kwantyl(edgeClasses.Length, lvlofDeg);
                        ...
                        if (pamOne > 0)
                        {
                            ...
                            if (ChiTestQ_rb.Checked)
                            {
                                ...
                                if (isGoodEdgeClasses)
                                {
                                    ...
                                    int[] tabCount = pear.licznoscKlas(size, tabFloat, edgeClasses.Length, edgeClasses);
                                    float[] tabPearJed =
                                        pear.licznoscTeoretycznaJedDys(size, edgeClasses.Length, edgeClasses, pamOne);
                                    float stat = pear.statystykaTestowaPearson(edgeClasses.Length, tabPearJed, tabCount);
                                    float quantile = pear.Kwantyl(edgeClasses.Length, lvlofDeg);
                    ...
                    else if (UniSt_rb.Checked)
                    {
                        ...
                        if (ChiTestQ_rb.Checked)
                        {
                            ...
                            if (isGoodEdgeClasses)
                            {
                                ...
                                float[] tabPearJed = pear.licznoscTeoretycznaJedStan(size, edgeClasses.Length, edgeClasses);
                                float stat = pear.statystykaTestowaPearson(edgeClasses.Length, tabPearJed, tabCount);
                                float quantile = pear.Kwantyl(edgeClasses.Length, lvlofDeg);
